...
async function syncIndex(opts = {}) {
  ...
  // mode: balanced | fill | newest | seed
  // seedItems: optional browser-collected roles [{id,name,cover_url|image,view_count,like_count}]
  const seedItems = Array.isArray(opts.seedItems) ? opts.seedItems.filter((x) => x && x.id != null) : [];
  const mode = opts.mode || (seedItems.length ? 'seed' : 'balanced');
  let newestPages = opts.newestPages != null ? opts.newestPages : (mode === 'fill' ? 1 : (mode === 'newest' ? 4 : 3));
  let crawlPages = opts.crawlPages != null ? opts.crawlPages : (mode === 'fill' ? 30 : (mode === 'newest' || mode === 'seed' ? 0 : 12));
  if (mode === 'newest' || mode === 'seed') {
    // only new uploads
    if (opts.newestPages == null && mode === 'newest') newestPages = 4;
    if (mode === 'seed') newestPages = 0;
    crawlPages = 0;
  }
  ...
  // Browser-seeded roles bypass Cloudflare-blocked server-side market list.
  if (seedItems.length) {
    await processPageData({ items: seedItems, total_pages: totalPages });
  }
