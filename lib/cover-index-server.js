...
async function syncIndex(opts = {}) {
  ...
  const newestPages = opts.newestPages != null ? opts.newestPages : (mode === 'fill' ? 1 : 3);
  const crawlPages = opts.crawlPages != null ? opts.crawlPages : (mode === 'fill' ? 30 : 12);
  ...
  if (done) advancedTo = 1; // restart continuous refresh loop
