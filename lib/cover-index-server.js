...
async function featuresFromImageBuffer(buf) {
  ...
  for (let y = 0; y < 8; y++) {
    for (let x = 0; x < 8; x++) {
      const lx = Math.floor(x * w / 9);
      const rx = Math.floor((x + 1) * w / 9);
      const py = Math.floor(y * h / 8);
      const li = (py * w + lx) * 3;
      const ri = (py * w + rx) * 3;
      const lv = 0.299 * data[li] + 0.587 * data[li + 1] + 0.114 * data[li + 2];
      const rv = 0.299 * data[ri] + 0.587 * data[ri + 1] + 0.114 * data[ri + 2];
      dhash[di++] = lv >= rv ? 1 : 0;
    }
  }
  ...
  return { ahashB64: ahashToB64(ahash), dhashB64: ahashToB64(dhash), colors };
}
...
async function syncIndex(opts = {}) {
  ...
  async function processPageData(data) {
    ...
    for (const role of items) {
      ...
      const old = map[key];
      if (old && old.image === proxied && old.ahash && old.dhash) continue;
      need.push(role);
    }
